										...
										int 			clientId			)
{
	...
	//! [ connect ]
	bool bRes = m_pClient->eConnect( 			host, 
												port, 
												clientId, 
												m_extraAuth 						);
	//! [ connect ]
...
void TestCppClient::realTimeBars()
{
	...
	//! [reqrealtimebars]
	m_pClient->reqRealTimeBars(				3001, 
											ContractSamples::EurGbpFx(), 
											5, 
											"MIDPOINT", 
											true, 
...
void TestCppClient::historicalDataRequests()
{
	...
	m_pClient->reqHistoricalData(			4001, 
											ContractSamples::EurGbpFx(), 
											queryTime, 
											"1 M", "1 day", "MIDPOINT", 
											1, 
	...
	m_pClient->reqHistoricalData(			4002, 
											ContractSamples::EuropeanStock(), 
											queryTime, 
											"10 D", "1 min", "TRADES", 
											1, 
...
void TestCppClient::optionsOperations()
{
	...
	//! [reqsecdefoptparams]
	m_pClient->reqSecDefOptParams(					0, 
													"IBM", 
													"", 
													"STK", 
													8314												);
	...
	m_pClient->calculateImpliedVolatility(			5001, 
													ContractSamples::OptionWithLocalSymbol(), 
													0.5, 
													55, 
													TagValueListSPtr()									);
	...
	//! [exercise_options]
	//** Exercising options ***
	m_pClient->exerciseOptions(						5003, 
...
void TestCppClient::marketScanners()
{
	...
	m_pClient->reqScannerSubscription(				7001, 
	...
	m_pClient->reqScannerSubscription(				7002, 
													ScannerSubscriptionSamples::HotUSStkByVolume(), 
													TagValueListSPtr(), 
	...
	m_pClient->reqScannerSubscription(				7003, 
...
void TestCppClient::fundamentals()
{
	...
	//! [reqfundamentaldata]
	m_pClient->reqFundamentalData(					8001, 
													ContractSamples::USStock(), 
													"ReportsFinSummary", 
													TagValueListSPtr()								);
	//! [reqfundamentaldata]
...
void TestCppClient::accountOperations()
{
	...
	//! [reqaaccountsummary]
	m_pClient->reqAccountSummary(			9001, 
											"All", 
	...
	m_pClient->reqAccountSummary(			9002, 
											"All", 
											"$LEDGER"											);
	//! [reqaaccountsummaryledger]
	...
	m_pClient->reqAccountSummary(			9003, 
											"All", 
											"$LEDGER:EUR"										);
	//! [reqaaccountsummaryledgercurrency]
	...
	m_pClient->reqAccountSummary(			9004, 
											"All", 
											"$LEDGER:ALL"										);
	//! [reqaaccountsummaryledgerall]
	...
	m_pClient->reqAccountUpdatesMulti(		9002, 
											"U150462", 
											"EUstocks", 
											true												);
	//! [reqaaccountupdatesmulti]
	...
	//! [reqpositionsmulti]
	m_pClient->reqPositionsMulti(			9003, 
											"U150462", 
											"EUstocks"											);
	//! [reqpositionsmulti]
...
void TestCppClient::orderOperations()
{
	...
	m_pClient->placeOrder(					m_orderId++, 
											ContractSamples::USStockAtSmart(), 
											OrderSamples::LimitOrderWithCashQty("BUY", 30, 5000)				);
	//! [place order with cashQty]
...
void TestCppClient::ocaSamples()
{
	...
	for( unsigned int i = 0; i < ocaOrders.size(); i++ )
	{
		...
		OrderSamples::OneCancelsAll(		"TestOca", 
											ocaOrders[ i ], 
											2																	);
...
void TestCppClient::conditionSamples()
{
	...
	ExecutionCondition* 	execCondition 		= 	dynamic_cast< ExecutionCondition *>(	OrderSamples::Execution_Condition(		"EUR.USD", 
																																	"CASH", 
																																	"IDEALPRO", 
																																	true			)	);
	...
	PercentChangeCondition* pctChangeCondition 	= 	dynamic_cast< PercentChangeCondition*>(	OrderSamples::Percent_Change_Condition(	15.0, 
																																	208813720, 
																																	"SMART", 
																																	true, 
																																	true			)	);
	
	TimeCondition* 			timeCondition 		= 	dynamic_cast< TimeCondition 	 *>(	OrderSamples::Time_Condition(			"20160118 23:59:59", 
																																	true, 
																																	false			)	);
	
	VolumeCondition* 		volumeCondition 	= 	dynamic_cast< VolumeCondition    *>(	OrderSamples::Volume_Condition(			208813720, 
																																	"SMART", 
																																	false, 
																																	100, 
																																	true			)	);
	...
	Order lmt2 = OrderSamples::LimitOrder( 				"BUY", 
														100, 
														20 												);
	...
	PriceCondition* priceCondition2 = dynamic_cast< PriceCondition *>(	OrderSamples::Price_Condition(		208813720, 
																											"SMART", 
																											600, 
																											false, 
																											false		));
...
void TestCppClient::bracketSample()
{
	...
	m_pClient->placeOrder(					parent.orderId, 
											ContractSamples::EuropeanStock(), 
											parent																	);
	
	m_pClient->placeOrder(					takeProfit.orderId, 
											ContractSamples::EuropeanStock(), 
											takeProfit																);

	m_pClient->placeOrder(					stopLoss.orderId, 
											ContractSamples::EuropeanStock(), 
											stopLoss																);
	//! [bracketsubmit]
...
void TestCppClient::hedgeSample()
{
	...
	Order parent = 	OrderSamples::LimitOrder(				"BUY", 
															100, 
															10								);
	...
	Order hedge = OrderSamples::MarketFHedge(				parent.orderId, 
															"BUY"							);
...
void TestCppClient::testAlgoSamples()
{
	...
	//! [algo_base_order]
	Order baseOrder = OrderSamples::LimitOrder(				"BUY", 
															1000, 
															1										);
	//! [algo_base_order]
	...
	AvailableAlgoParams::FillAccumulateDistributeParams(				baseOrder, 
																		10, 
																		60, 
																		true, 
																		true, 
	...
	//! [twap]
	AvailableAlgoParams::FillTwapParams(								baseOrder, 
																		"Marketable", 
																		"09:00:00 CET", 
																		"16:00:00 CET", 
																		true, 
	...
	//! [vwap]
	AvailableAlgoParams::FillBalanceImpactRiskParams(					baseOrder, 
																		0.1, 
																		"Aggressive", 
																		true									);
	...
	//! [minimpact]
	AvailableAlgoParams::FillMinImpactParams(							baseOrder, 
																		0.3										);
	...
	//! [adaptive]
	AvailableAlgoParams::FillAdaptiveParams(							baseOrder, 
																		"Normal"								);
	...
	//! [closepx]
    AvailableAlgoParams::FillClosePriceParams(							baseOrder, 
																		0.5, 
																		"Neutral", 
																		"12:00:00 EST", 
																		true, 
    ...
    //! [pctvol]
    AvailableAlgoParams::FillPctVolParams(								baseOrder, 
																		0.5, 
																		"12:00:00 EST", 
																		"14:00:00 EST", 
																		true, 
	...
	m_pClient->placeOrder(												m_orderId++, 
																		ContractSamples::JefferiesContract(), 
																		baseOrder								);
	//! [jeff_vwap_algo]
...
void TestCppClient::reqMatchingSymbols()
{
	...
	//! [reqmatchingsymbols]
	m_pClient->reqMatchingSymbols(					11001, 	
													"IBM"							);
...
void TestCppClient::reqSmartComponents()
{
	...
	if ( m_bboExchange.size() > 0 ) 
	{
		...
		//! [reqsmartcomponents]
		m_pClient->reqSmartComponents(		13002, 
											m_bboExchange										);
...
void TestCppClient::reqNewsArticle()
{
	...
	m_pClient->reqNewsArticle(				12001, 
											"MST", 
											"MST$06f53098", 
...
void TestCppClient::reqHistoricalNews()
{
	...
	m_pClient->reqHistoricalNews(			12001, 
											8314, 
											"BZ+FLY", 
											"", 
											"", 
...
void TestCppClient::reqHeadTimestamp() 
{
	...
	//! [reqHeadTimeStamp]
	m_pClient->reqHeadTimestamp(			14001, 
											ContractSamples::EurGbpFx(), 
											"MIDPOINT", 
											1, 
											1													);
...
void TestCppClient::reqHistogramData() 
{
	...
	//! [reqHistogramData]
	m_pClient->reqHistogramData(			15001, 
...
void TestCppClient::rerouteCFDOperations()
{
    ...
    //! [reqmktdatacfd]
	m_pClient->reqMktData(					16001, 
											ContractSamples::USStockCFD(), 
											"", 
											false, 
											false, 
											TagValueListSPtr()									);
	...
	m_pClient->reqMktData(					16002, 
											ContractSamples::EuropeanStockCFD(), 
											"", 
											false, 
											false, 
											TagValueListSPtr()									);
	...
	m_pClient->reqMktData(					16003, 
											ContractSamples::CashCFD(), 
											"", 
											false, 
											false, 
											TagValueListSPtr()									);
...
void TestCppClient::marketRuleOperations()
{
	...
	m_pClient->reqContractDetails(			17002, 
											ContractSamples::IBKRStk()							);
...
void TestCppClient::continuousFuturesOperations()
{
	...
	m_pClient->reqHistoricalData(			18002, 
											ContractSamples::ContFut(), 
											queryTime, 
											"1 Y", 
											"1 month", 
											...
											const OrderState& orderState					) 
{
	...
	"LmtPrice: %g, AuxPrice: %g, Status: %s\n", 
											...
											order.totalQuantity, 
											order.cashQty == UNSET_DOUBLE ? 0 : order.cashQty, 
											order.lmtPrice,
											order.auxPrice, 
												...
												const std::string& 		articleText					) 
{
	...
	else if ( articleType == 1 ) 
	{
		...
		outfile.write(						(const char*)bytes.data(), 
											bytes.size()									);
...
void TestCppClient::marketRule(int marketRuleId, const std::vector<PriceIncrement> &priceIncrements) 
{
	...
	for( unsigned int i = 0; i < priceIncrements.size(); i++ ) 
	{
		...
		printf(					"Low Edge: %g, Increment: %g\n", 
								priceIncrements[ i ].lowEdge, 
								priceIncrements[ i ].increment					);
												...
												const OrderState& 	orderState				) 
{
	...
	printf( 				"CompletedOrder. PermId: %i, ParentPermId: %lld, Account: %s, Symbol: %s, SecType: %s, Exchange: %s:, Action: %s, OrderType: %s, TotalQty: %g, CashQty: %g, FilledQty: %g, "
		"LmtPrice: %g, AuxPrice: %g, Status: %s, CompletedTime: %s, CompletedStatus: %s\n", 
							...
							order.auxPrice, 
							orderState.status.c_str(), 
							orderState.completedTime.c_str(), 
