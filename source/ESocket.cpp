...
int ESocket::send( 	EMessage  *pMsg  ) 
{
    ...
    return bufferedSend(			pMsg->begin(), 
									pMsg->end() - pMsg->begin()			);
...
int ESocket::bufferedSend(			const char* 	buf, 
									size_t 			sz				)
{
	...
	if( !m_outBuffer.empty() ) 
	{
		...
		m_outBuffer.insert( 			m_outBuffer.end(), 
										buf, 
										buf + sz					);
...
int ESocket::sendBufferedData()
{
	...
	int nResult = send( 				&m_outBuffer[ 0 ], 
										m_outBuffer.size()					);
...
int ESocket::send( 			const char* 	buf, 
							size_t 			sz				)
{
	...
	//*********************************************************
	// SYSTEM CALL 
	// send data via Socket: send sz bytes of buf to socket FD
	//*********************************************************

	int nResult = ::send( 				m_fd, 
										buf, 
										sz, 
										0					);
...
void ESocket::CleanupBuffer(		std::vector<char>& 		buffer, 
									int 					processed		)
{
